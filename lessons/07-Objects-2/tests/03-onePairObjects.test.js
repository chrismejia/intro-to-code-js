...
describe("#3: onePairObjects", function () {
  ...
  describe("returns the correct number of array entries", function () {
    it("when inputObj is empty", function () {
      expect(onePairObjects(noPairs).length).to.equal(0);
    });

    it("when inputObj has one key-value pair", function () {
      onePairVariety.forEach((singleObj) => {
        expect(onePairObjects(singleObj).length).to.equal(1);
      });
    });

    it("when inputObj has multiple key-value pairs", function () {
      expect(onePairObjects(multiPairObj).length).to.equal(3);
    });
  });

  describe("returns the correct array entries in the correct order", function () {
    it("when inputObj is empty", function () {
