...
xdescribe("#2: repeatStr", () => {
  ...
  it("does not call on String.repeat()", () => {
    counts.forEach((count) => {
      const strRepeatSpy = sinon.spy(String.prototype, "repeat");

      wrapper.repeatStr("peko", count);

      const calls = strRepeatSpy.callCount;

      expect(calls).to.equal(0);

      strRepeatSpy.restore();
    });
  });
});
