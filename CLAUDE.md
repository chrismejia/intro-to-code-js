...
- `0X-Guide` is instructor-facing. It may contain answer-bearing files,
  maintainer docs, `AGENTS.md`, `CLAUDE.md`, `base/`, `teaching-notes/`, and
  `wip-problems/`.
...
- Upstream `main` is student-facing. Its current tree should not contain
  answers, maintainer docs, `AGENTS.md`, `CLAUDE.md`, `base/`,
...
- Issue branch PRs for the Jest migration feature target
  `feat/173-migrate-repo-testing-to-jest`.
- Commit messages in this migration work should include issue references, for
  example `#173 #203`; final issue commits should include `Closes #203` when
  appropriate.
...
- Do not add answers, maintainer docs, or agent guidance files to upstream
  `main`.
- Do not remove `teaching-notes/`, `wip-problems/`, `base/`, or filled answers while working on `0X-Guide` unless explicitly asked.
